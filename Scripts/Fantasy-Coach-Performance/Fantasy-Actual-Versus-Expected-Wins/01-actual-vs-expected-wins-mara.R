...
library(ggthemes)
library(pracma)
all_fixtures <- data.frame(perms(c(2,3,4,5,6,7,8)))
...
all_results <-
  all_fixtures %>%
  mutate(`Round 1` = ifelse(`Round 1` < 1583, TRUE, FALSE)) %>%
  mutate(`Round 2` = ifelse(`Round 2` < 1564, TRUE, FALSE)) %>%
  mutate(`Round 3` = ifelse(`Round 3` < 1500, TRUE, FALSE)) %>%
  mutate(`Round 4` = ifelse(`Round 4` < 1684, TRUE, FALSE)) %>%
  mutate(`Round 5` = ifelse(`Round 5` < 1635, TRUE, FALSE)) %>%
  mutate(`Round 6` = ifelse(`Round 6` < 1543, TRUE, FALSE)) %>%
  mutate(`Round 7` = ifelse(`Round 7` < 1640, TRUE, FALSE)) %>%
  mutate(`Round 8` = ifelse(`Round 8` < 1551, TRUE, FALSE)) %>%
  mutate(`Round 9` = ifelse(`Round 9` < 1640, TRUE, FALSE)) %>%
  mutate(`Round 10` = ifelse(`Round 10` < 1551, TRUE, FALSE)) %>%
  mutate(`Round 11` = ifelse(`Round 11` < 1542, TRUE, FALSE)) %>%
  mutate(`Round 15` = ifelse(`Round 15` < 1493, TRUE, FALSE)) %>%
  mutate(`Round 16` = ifelse(`Round 16` < 1465, TRUE, FALSE))
...
all_results %>%
  ...
  scale_x_continuous(limits = c(4,13), breaks = seq(0, 13, by = 1)) +
  ggtitle("Alex Marateo", subtitle = "Number of wins over all possible permutations of the fixtures") +
  theme_fivethirtyeight()

# Save plot to output folder
ggsave("Output/expected_vs_observed_wins_mara.png")
