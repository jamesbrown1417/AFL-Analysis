...
library(ggthemes)
library(pracma)
all_fixtures <- data.frame(perms(c(1,2,3,4,6,7,8)))
...
all_results <-
  all_fixtures %>%
  mutate(`Round 1` = ifelse(`Round 1` < 1411, TRUE, FALSE)) %>%
  mutate(`Round 2` = ifelse(`Round 2` < 1493, TRUE, FALSE)) %>%
  mutate(`Round 3` = ifelse(`Round 3` < 1482, TRUE, FALSE)) %>%
  mutate(`Round 4` = ifelse(`Round 4` < 1597, TRUE, FALSE)) %>%
  mutate(`Round 5` = ifelse(`Round 5` < 1379, TRUE, FALSE)) %>%
  mutate(`Round 6` = ifelse(`Round 6` < 1500, TRUE, FALSE)) %>%
  mutate(`Round 7` = ifelse(`Round 7` < 1243, TRUE, FALSE)) %>%
  mutate(`Round 8` = ifelse(`Round 8` < 1391, TRUE, FALSE)) %>%
  mutate(`Round 9` = ifelse(`Round 9` < 1441, TRUE, FALSE)) %>%
  mutate(`Round 10` = ifelse(`Round 10` < 1420, TRUE, FALSE)) %>%
  mutate(`Round 11` = ifelse(`Round 11` < 1422, TRUE, FALSE)) %>%
  mutate(`Round 15` = ifelse(`Round 15` < 1550, TRUE, FALSE)) %>%
  mutate(`Round 16` = ifelse(`Round 16` < 1307, TRUE, FALSE))
...
# Plot
all_results %>%
  ggplot(mapping = aes(x = `Total Wins`, fill = `Total Wins` == 2)) +
  ...
  ggtitle("Nick Kirchner", subtitle = "Number of wins over all possible permutations of the fixtures") +
  theme_fivethirtyeight()

# Save plot to output folder
ggsave("Output/expected_vs_observed_wins_kirch.png")
