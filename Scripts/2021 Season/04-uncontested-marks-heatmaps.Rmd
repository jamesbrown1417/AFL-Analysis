---
title: "Uncontested Mark Heatmaps - 2021"
author: "James Brown"
date: "`r Sys.Date()`"
output:
  html_document:
...
```{r}
# Update combined stats table to have total uncontested marks by team and player's percentage contribution
combined_stats_table <-
combined_stats_table %>%
  group_by(team, round_number) %>%
  mutate(total_uncontested_marks = sum(uncontestedMarks)) %>%
  ungroup() %>%
  mutate(uncontested_marks_percentage = round((uncontestedMarks / total_uncontested_marks)*100, digits = 2))
```
...
uncontested_marks <-
  ...
  select(`Player Name` = player_name, uncontested_marks_percentage, round_number) %>%
  group_by(`Player Name`) %>%
  filter(max(uncontested_marks_percentage, na.rm = TRUE) > 0) %>%
  ungroup() %>%
  arrange(round_number) %>%
  pivot_wider(id_cols = `Player Name`, names_from = round_number, values_from = uncontested_marks_percentage)

uncontested_marks_avgs <-
combined_stats_table %>%
  filter(team == "Adelaide Crows") %>%
  group_by(player_name) %>%
  summarise(`Mean Uncontested Mark Percentage` = mean(uncontested_marks_percentage, na.rm = TRUE)) %>%
  rename(`Player Name` = player_name)

heatmap <- uncontested_marks %>% left_join(uncontested_marks_avgs) %>% arrange(desc(`Mean Uncontested Mark Percentage`))
