---
title: "Rebound 50 Heatmaps - 2021"
author: "James Brown"
date: "`r Sys.Date()`"
output:
  html_document:
...
rebound_50s <-
  ...
  select(`Player Name` = player_name, rebound_50s_percentage, round_number) %>%
  group_by(`Player Name`) %>%
  filter(max(rebound_50s_percentage, na.rm = TRUE) > 0) %>%
  ungroup() %>%
  arrange(round_number) %>%
  pivot_wider(id_cols = `Player Name`, names_from = round_number, values_from = rebound_50s_percentage)

rebound_50s_avgs <-
combined_stats_table %>%
  filter(team == "Adelaide Crows") %>%
  group_by(player_name) %>%
  summarise(`Mean Rebound 50 Percentage` = mean(rebound_50s_percentage, na.rm = TRUE)) %>%
  rename(`Player Name` = player_name)

heatmap <- rebound_50s %>% left_join(rebound_50s_avgs) %>% arrange(desc(`Mean Rebound 50 Percentage`))
