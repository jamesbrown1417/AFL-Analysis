...
library(ggthemes)
library(pracma)
all_fixtures <- data.frame(perms(c(1,2,3,4,5,7,8)))
...
all_results <-
  all_fixtures %>%
  mutate(`Round 1` = ifelse(`Round 1` < 1441, TRUE, FALSE)) %>%
  mutate(`Round 2` = ifelse(`Round 2` < 1582, TRUE, FALSE)) %>%
  mutate(`Round 3` = ifelse(`Round 3` < 1562, TRUE, FALSE)) %>%
  mutate(`Round 4` = ifelse(`Round 4` < 1577, TRUE, FALSE)) %>%
  mutate(`Round 5` = ifelse(`Round 5` < 1528, TRUE, FALSE)) %>%
  mutate(`Round 6` = ifelse(`Round 6` < 1480, TRUE, FALSE)) %>%
  mutate(`Round 7` = ifelse(`Round 7` < 1528, TRUE, FALSE)) %>%
  mutate(`Round 8` = ifelse(`Round 8` < 1467, TRUE, FALSE)) %>%
  mutate(`Round 9` = ifelse(`Round 9` < 1414, TRUE, FALSE)) %>%
  mutate(`Round 10` = ifelse(`Round 10` < 1464, TRUE, FALSE)) %>%
  mutate(`Round 11` = ifelse(`Round 11` < 1522, TRUE, FALSE)) %>%
  mutate(`Round 15` = ifelse(`Round 15` < 1618, TRUE, FALSE)) %>%
  mutate(`Round 16` = ifelse(`Round 16` < 1731, TRUE, FALSE))
...
# Plot
all_results %>%
  ggplot(mapping = aes(x = `Total Wins`, fill = `Total Wins` == 5)) +
  geom_histogram(binwidth = 1, colour = "black") +
  scale_fill_manual(values = c("grey", "green"), name = "", labels = c("Potential Wins", "Observed Wins")) +
  scale_x_continuous(limits = c(1,11), breaks = seq(0, 9, by = 1)) +
  ggtitle("James Brown", subtitle = "Number of wins over all possible permutations of the fixtures") +
  theme_fivethirtyeight()

# Save plot to output folder
ggsave("Output/expected_vs_observed_wins_brown.png")
