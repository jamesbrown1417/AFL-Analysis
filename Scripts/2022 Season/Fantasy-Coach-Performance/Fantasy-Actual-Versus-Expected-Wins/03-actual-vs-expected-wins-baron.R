...
library(ggthemes)
library(pracma)
all_fixtures <- data.frame(perms(c(1,2,4,5,6,7,8)))
...
all_results <-
  all_fixtures %>%
  mutate(`Round 1` = ifelse(`Round 1` < 1500, TRUE, FALSE)) %>%
  mutate(`Round 2` = ifelse(`Round 2` < 1540, TRUE, FALSE)) %>%
  mutate(`Round 3` = ifelse(`Round 3` < 1518, TRUE, FALSE)) %>%
  mutate(`Round 4` = ifelse(`Round 4` < 1475, TRUE, FALSE)) %>%
  mutate(`Round 5` = ifelse(`Round 5` < 1546, TRUE, FALSE)) %>%
  mutate(`Round 6` = ifelse(`Round 6` < 1784, TRUE, FALSE)) %>%
  mutate(`Round 7` = ifelse(`Round 7` < 1624, TRUE, FALSE)) %>%
  mutate(`Round 8` = ifelse(`Round 8` < 1694, TRUE, FALSE)) %>%
  mutate(`Round 9` = ifelse(`Round 9` < 1726, TRUE, FALSE)) %>%
  mutate(`Round 10` = ifelse(`Round 10` < 1665, TRUE, FALSE)) %>%
  mutate(`Round 11` = ifelse(`Round 11` < 1731, TRUE, FALSE)) %>%
  mutate(`Round 15` = ifelse(`Round 15` < 1677, TRUE, FALSE)) %>%
  mutate(`Round 16` = ifelse(`Round 16` < 1696, TRUE, FALSE))
...
# Plot
all_results %>%
  ggplot(mapping = aes(x = `Total Wins`, fill = `Total Wins` == 9)) +
  geom_histogram(binwidth = 1, colour = "black") +
  scale_fill_manual(values = c("grey", "green"), name = "", labels = c("Potential Wins", "Observed Wins")) +
  scale_x_continuous(limits = c(4,14), breaks = seq(0, 13, by = 1)) +
  ggtitle("Matt Baron", subtitle = "Number of wins over all possible permutations of the fixtures") +
  theme_fivethirtyeight()

# Save plot to output folder
ggsave("Output/expected_vs_observed_wins_baron.png")
