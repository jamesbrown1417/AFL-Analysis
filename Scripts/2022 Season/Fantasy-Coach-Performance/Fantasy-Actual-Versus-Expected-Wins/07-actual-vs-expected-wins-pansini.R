...
library(ggthemes)
library(pracma)
all_fixtures <- data.frame(perms(c(1,2,3,4,5,6,8)))
...
all_results <-
  all_fixtures %>%
  mutate(`Round 1` = ifelse(`Round 1` < 1502, TRUE, FALSE)) %>%
  mutate(`Round 2` = ifelse(`Round 2` < 1549, TRUE, FALSE)) %>%
  mutate(`Round 3` = ifelse(`Round 3` < 1523, TRUE, FALSE)) %>%
  mutate(`Round 4` = ifelse(`Round 4` < 1362, TRUE, FALSE)) %>%
  mutate(`Round 5` = ifelse(`Round 5` < 1524, TRUE, FALSE)) %>%
  mutate(`Round 6` = ifelse(`Round 6` < 1668, TRUE, FALSE)) %>%
  mutate(`Round 7` = ifelse(`Round 7` < 1589, TRUE, FALSE)) %>%
  mutate(`Round 8` = ifelse(`Round 8` < 1506, TRUE, FALSE)) %>%
  mutate(`Round 9` = ifelse(`Round 9` < 1701, TRUE, FALSE)) %>%
  mutate(`Round 10` = ifelse(`Round 10` < 1489, TRUE, FALSE)) %>%
  mutate(`Round 11` = ifelse(`Round 11` < 1594, TRUE, FALSE)) %>%
  mutate(`Round 15` = ifelse(`Round 15` < 1534, TRUE, FALSE)) %>%
  mutate(`Round 16` = ifelse(`Round 16` < 1649, TRUE, FALSE))
...
# Plot
all_results %>%
  ggplot(mapping = aes(x = `Total Wins`, fill = `Total Wins` == 6)) +
  geom_histogram(binwidth = 1, colour = "black") +
  scale_fill_manual(values = c("grey", "green"), name = "", labels = c("Potential Wins", "Observed Wins")) +
  scale_x_continuous(limits = c(2,10), breaks = seq(2, 9, by = 1)) +
  ggtitle("Nic Pansini", subtitle = "Number of wins over all possible permutations of the fixtures") +
  theme_fivethirtyeight()

# Save plot to output folder
ggsave("Output/expected_vs_observed_wins_pansini.png")
