...
library(ggthemes)
library(pracma)
all_fixtures <- data.frame(perms(1:7))
...
all_results <-
  all_fixtures %>%
  mutate(`Round 1` = ifelse(`Round 1` < 1521, TRUE, FALSE)) %>%
  mutate(`Round 2` = ifelse(`Round 2` < 1586, TRUE, FALSE)) %>%
  mutate(`Round 3` = ifelse(`Round 3` < 1562, TRUE, FALSE)) %>%
  mutate(`Round 4` = ifelse(`Round 4` < 1532, TRUE, FALSE)) %>%
  mutate(`Round 5` = ifelse(`Round 5` < 1590, TRUE, FALSE)) %>%
  mutate(`Round 6` = ifelse(`Round 6` < 1577, TRUE, FALSE)) %>%
  mutate(`Round 7` = ifelse(`Round 7` < 1547, TRUE, FALSE)) %>%
  mutate(`Round 8` = ifelse(`Round 8` < 1645, TRUE, FALSE)) %>%
  mutate(`Round 9` = ifelse(`Round 9` < 1535, TRUE, FALSE)) %>%
  mutate(`Round 10` = ifelse(`Round 10` < 1521, TRUE, FALSE)) %>%
  mutate(`Round 11` = ifelse(`Round 11` < 1603, TRUE, FALSE)) %>%
  mutate(`Round 15` = ifelse(`Round 15` < 1696, TRUE, FALSE)) %>%
  mutate(`Round 16` = ifelse(`Round 16` < 1704, TRUE, FALSE))
...
all_results %>%
  ...
  scale_x_continuous(limits = c(5,13), breaks = seq(6, 13, by = 1)) +
  ggtitle("John Rossi", subtitle = "Number of wins over all possible permutations of the fixtures") +
  theme_fivethirtyeight()

# Save plot to output folder
ggsave("Output/expected_vs_observed_wins_rossi.png")
