...
#                                                          #
##%######################################################%##
all_fielded_teams_full$round_number <-
  factor(all_fielded_teams_full$round_number,
         levels = c("Round 1",
                    "Round 2",
                    "Round 3",
                    "Round 4",
...
## Highest and Lowest Fielded Scores

#### Injuries and non-playing players excluded
...
all_fielded_teams_full %>%
  select(`Player` = player_name_full,
         `Score` = fantasy_points,
          TOG,
         `Team` = fantasy_team,
         Round = round_number,
         Position = fielded_position) %>%
  arrange(desc(Score)) %>%
  head(10)
```
...
all_fielded_teams_full %>% 
  select(Team = fantasy_team,
         Score = fantasy_points,
         TOG,
         Player = player_name_full,
         Round = round_number,
         Position = fielded_position) %>%
  arrange(desc(Score)) %>%
  group_by(Team) %>%
...
## Outlier Scores (Biggest Deviation From Overall Average)
...
all_fielded_teams_full %>%
  select(Team = fantasy_team,
         ...
         Round = round_number,
         Position = fielded_position,
         `Season Avg` = average_score) %>%
  mutate(Diff = Score - `Season Avg`) %>%
  arrange(desc(Diff)) %>%
  head(10)
```
...
# loophole performance
  ...
  select(Team = fantasy_team, `Number of Loopholed Players` = number_of_loopholes_used, `Avg Loophole Score` = avg_loopholed_score)

```

## Players Who Featured for the Most Teams
...
all_fielded_teams_full %>%
  arrange(player_name_full, round_number) %>%
  group_by(player_name_full, fantasy_team) %>%
  summarise(`Games Played` = n()) %>%
  group_by(player_name_full) %>%
  filter(n() > 1) %>%
  mutate( rn = row_number()) %>%
  ungroup() %>%
  transmute(`Player` = player_name_full,
         `Games Played`,
         `Team` = fantasy_team,
         overall_max_rn = max(rn)) %>%
  group_by(Player) %>%
  filter(max(row_number()) == overall_max_rn) %>%
  mutate(rn = row_number()) %>%
  pivot_wider(id_cols = Player, names_from = rn, values_from = Team, names_prefix = "Team")

```

## Best 5 Players By Team

###### Min 5 games played, only games where player contributed to overall score and were not injured are counted
...
best_5_by_team <-
  all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" &
           !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  group_by(player_name_full, fantasy_team) %>%
  summarise(`Avg Score` = mean(fantasy_points, na.rm = TRUE),
            `Games Played` = n()) %>%
  ungroup() %>%
  transmute(`Team` = fantasy_team,
            `Player` = player_name_full,
            `Games Played`,
            `Avg Score`) %>%
  group_by(Player, Team) %>%
  arrange(Team, desc(`Avg Score`)) %>%
  group_by(Team) %>%
...
best_5_by_team %>%
  ...
  ungroup() %>%
  select(-Team) %>%
  mutate(`Avg Score` = round(`Avg Score`, 2))
```
...
all_fielded_teams_full %>%
  ...
  group_by(player_name_full, fantasy_team) %>%
  summarise(`Avg Score` = mean(fantasy_points, na.rm = TRUE), `Games Played` = n())%>%
  ungroup() %>%
  transmute(`Player` = player_name_full,
         `Games Played`,
         `Avg Score`,
...
##### Based on player averages, with injuries (\<50 % TOG) and bye rounds excluded
...
combined_stats_table %>%
  filter(round_number %notin% c("Round 12", "Round 13", "Round 14")) %>%
  filter(round_number < "Round 18") %>%
  filter(TOG >= 50) %>%
  ...
  head(5) %>%
  select(Player = player_name, Avg = avg_score) %>%
...
combined_stats_table %>%
  ...
  filter(round_number < "Round 18") %>%
  filter(TOG >= 50) %>%
  filter(fantasy_midfield_status & !fantasy_forward_status) %>%
  group_by(player_name) %>%
  summarise(avg_score = mean(fantasy_points, na.rm = TRUE)) %>%
  arrange(desc(avg_score)) %>%
  head(8) %>%
...
combined_stats_table %>%
  ...
  filter(round_number < "Round 18") %>%
  filter(TOG >= 50) %>%
  filter(fantasy_ruck_status & !fantasy_forward_status) %>%
  group_by(player_name) %>%
  summarise(avg_score = mean(fantasy_points, na.rm = TRUE)) %>%
  arrange(desc(avg_score)) %>%
  head(1) %>%
...
bind_rows(def_interchange, mid_interchange, ruck_interchange, forward_interchange) %>%
  mutate(Avg = round(Avg, 2))
```
:::
...
all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" & !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  filter(str_detect(fielded_position, "Defender")) %>%
  group_by(fantasy_team) %>%
  summarise(`Total Points` = sum(fantasy_points)) %>%
  arrange(desc(`Total Points`))

midfielder_totals <-
all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" & !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  filter(str_detect(fielded_position, "Midfielder")) %>%
  group_by(fantasy_team) %>%
  summarise(`Total Points` = sum(fantasy_points)) %>%
  arrange(desc(`Total Points`))
...
all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" & !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  filter(str_detect(fielded_position, "Ruck")) %>%
  group_by(fantasy_team) %>%
  summarise(`Total Points` = sum(fantasy_points)) %>%
  arrange(desc(`Total Points`))
...
all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" & !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  filter(str_detect(fielded_position, "Forward")) %>%
  group_by(fantasy_team) %>%
  summarise(`Total Points` = sum(fantasy_points)) %>%
  arrange(desc(`Total Points`))
```
...
defender_totals %>%
  ggplot(mapping = aes(x = reorder(fantasy_team, `Total Points`), y = `Total Points`)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(5250, 6500)) +
  xlab("Team") +
  ggtitle("Total Points from Defenders") +
...
midfielder_totals %>%
  ggplot(mapping = aes(x = reorder(fantasy_team, `Total Points`), y = `Total Points`)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(8500, 9750)) +
  xlab("Team") +
  ggtitle("Total Points from Midfielders") +
  theme_fivethirtyeight()
```
...
ruck_totals %>%
  ggplot(mapping = aes(x = reorder(fantasy_team, `Total Points`), y = `Total Points`)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(1100, 1400)) +
  xlab("Team") +
  ggtitle("Total Points from Ruckman") +
  theme_fivethirtyeight()
```
...
forward_totals %>%
  ggplot(mapping = aes(x = reorder(fantasy_team, `Total Points`), y = `Total Points`)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(4500, 6250)) +
  xlab("Team") +
  ggtitle("Total Points from Forwards") +
  theme_fivethirtyeight()
```
:::
...
all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" & !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  filter(fantasy_points >= 140) %>%
  group_by(fantasy_team) %>%
  tally() %>%
  arrange(desc(n)) %>%
  ggplot(mapping = aes(x = reorder(fantasy_team, n), y = n)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(0, 9)) +
  scale_y_continuous(breaks = seq(0, 9, by = 1)) +
  xlab("Team") +
  ggtitle("Number of Players Scoring 140+") +
  theme_fivethirtyeight()
```
...
all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" & !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  filter(fantasy_points >= 120) %>%
  group_by(fantasy_team) %>%
  tally() %>%
  arrange(desc(n)) %>%
  ggplot(mapping = aes(x = reorder(fantasy_team, n), y = n)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(12, 35)) +
  scale_y_continuous(breaks = seq(12, 36, by = 2)) +
  xlab("Team") +
  ggtitle("Number of Players Scoring 120+") +
  theme_fivethirtyeight()
```
...
all_fielded_teams_full %>%
  ...
  xlab("Team") +
  ggtitle("Number of Players Scoring 100+") +
  theme_fivethirtyeight()
```
...
all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" & !injury_substituted & TOG > 0 & !is.na(TOG)) %>%
  filter(fantasy_points < 70) %>%
  group_by(fantasy_team) %>%
  tally() %>%
  arrange(desc(n)) %>%
  ggplot(mapping = aes(x = reorder(fantasy_team, n), y = n)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(45, 105)) +
  scale_y_continuous(breaks = seq(45, 105, by = 5)) +
  xlab("Team") +
  ggtitle("Number of Players Scoring <70") +
  theme_fivethirtyeight()
```
...
all_fielded_teams_full %>%
  ...
  xlab("Team") +
  ggtitle("Number of Players Scoring <60") +
  theme_fivethirtyeight()
```
...
all_fielded_teams_full %>%
  ...
  ggplot(mapping = aes(x = reorder(fantasy_team, n), y = n)) +
  geom_col(fill = "orangered", colour = "black", alpha = 0.7) +
  coord_flip(ylim = c(8, 28)) +
  scale_y_continuous(breaks = seq(8, 28, by = 2)) +
  xlab("Team") +
  ggtitle("Number of Players Scoring <50") +
  theme_fivethirtyeight()
```
:::

## Total Players Fielded
...
all_fielded_teams_full %>%
  group_by(fantasy_team) %>%
  distinct(player_name, fantasy_team, .keep_all = TRUE) %>%
  group_by(fantasy_team) %>%
  tally() %>%
  arrange(desc(n)) %>%
  select(Team = fantasy_team, `Total Players Fielded` = n)
...
games_missed <-
  ...
  mutate(games_missed = 14 - games_played)
...
# Get total games missed by top draft picks from first 5 and 10 rounds
first_5_player_games_missed <-
draft_picks_games_missed %>%
  filter(pick <= 40) %>%
  group_by(team) %>%
...
## Games Missed By Draft Picks

::: panel-tabset
## First 5 Players Drafted
...
first_5_player_games_missed %>%
  transmute(Team = team,
         `Games Missed` = player_games_missed,
         `%` = 100 - percentage_games_from_picks)
```

## First 10 Players Drafted
...
## On-field Injuries (\< 50% TOG)

```{r}
#| include: false

uncovered_onfield_injuries <-
  all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" &
           !injury_substituted &
           TOG > 0 &
           !is.na(TOG) &
           TOG < 50) %>%
  filter(str_detect(fielded_position, "Emergency", negate = TRUE))

covered_onfield_injuries <- 
  all_fielded_teams_full %>%
  filter(fielded_position != "Interchange" &
           injury_substituted &
           TOG > 0 &
...
## Number of On-field Injuries (\< 50% TOG) By Team
...
covered_onfield_injuries %>%
  ...
  tally() %>%
  arrange(desc(n)) %>%
  select(Team = fantasy_team, `Number of On-field Injuries` = n) %>%
  full_join(defender_totals %>% select(Team = fantasy_team)) %>%
  replace_na(list("Number of On-field Injuries" = 0))
```
